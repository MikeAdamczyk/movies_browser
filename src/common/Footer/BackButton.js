...
export const BackButton = ({firstTitle, secondTitle}) => {
    ...
    return (
        window.location.href.includes("people") ?
            <>
                <ChangePageButton onClick={() => {
                    dispatch(setPeopleFirstPage())
                    ...
                    < ButtonTitle
                        ...
                        inactive={peopleCurrentPage === 1}
                    >
                        {firstTitle}
                    </ButtonTitle>
                </ChangePageButton>
                <ChangePageButton onClick={() => dispatch(setPeoplePreviousPage())} directionChange
                                  disabled={peopleCurrentPage === 1}>
                    {peopleCurrentPage === 1 ?
                        <Arrow src={disabled}/>
                        :
            ...
            <>
                <ChangePageButton onClick={() => {
                    ...
                    < ButtonTitle
                        directionChange="left"
                        inactive={moviesCurrentPage === 1}
                    >
                        {firstTitle}
                    </ButtonTitle>
                </ChangePageButton>
                <ChangePageButton onClick={() => dispatch(setPreviousPage())} directionChange
                                  disabled={moviesCurrentPage === 1}>
                    {moviesCurrentPage === 1 ?
                        <Arrow src={disabled}/>
                        :
